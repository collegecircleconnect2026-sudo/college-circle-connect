...
export default function MentorDashboard({ profile }: { profile: any }) {
  ...
  async function handleRequest(matchId: string, action: 'active' | 'declined') {
    await supabase
      ...
      .eq('id', matchId);
    if (mentorProfile) {
      fetchRequests(mentorProfile.id);
      fetchMatches(mentorProfile.id);
    }
  }
    ...
    <div style={s.app}>
      <div style={s.sidebar}>
        ...
        <div style={{ padding: '8px 18px 0' }}>
          <span style={s.pill}>Mentor portal</span>
        </div>
        <div style={s.nav}>
          <div style={s.navSection}>Mentoring</div>
          <div
            style={s.navItem(page === 'requests')}
            onClick={() => setPage('requests')}
          >
            Incoming requests
          </div>
          <div
            style={s.navItem(page === 'impact')}
      ...
      <div style={s.main}>
        {page === 'requests' && (
          <>
            <div style={s.ph}>
              <div style={s.pt}>Incoming requests</div>
              <div style={s.ps}>Students who want to connect with you</div>
            </div>
            {requests.length === 0 && (
            ...
            {requests.map((r) => (
              <div key={r.id} style={s.reqRow}>
                <div
                  style={{
                    ...s.av('rgba(212,160,23,0.2)', C.goldLight),
                    marginBottom: 0,
                  }}
                >
                  {r.users?.avatar_initials || '??'}
                </div>
                <div style={{ flex: 1 }}>
                  <div
                  ...
                  <div style={{ fontSize: 12, color: C.muted }}>
                    {r.users?.field_of_interest || 'Student'}
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 6 }}>
                  <button
                    style={s.btn(true)}
                    onClick={() => handleRequest(r.id, 'active')}
                  >
                    Accept
                  </button>
                  <button
              ...
              <>
                ...
                {matches.map((m) => (
                  <div key={m.id} style={s.reqRow}>
                    <div
                      style={{
                        ...s.av('rgba(93,202,165,0.2)', '#5DCAA5'),
                        marginBottom: 0,
                      }}
                    >
        ...
        {page === 'impact' && (
          <>
            <div style={s.ph}>
              <div style={s.pt}>My impact</div>
              <div style={s.ps}>
                The difference you are making at Hartford Memorial
              </div>
            </div>
            <div style={s.impactGrid}>
              <div style={s.impactItem}>
                <div style={s.impactBig}>{matches.length}</div>
                <div style={s.impactLabel}>Active mentees</div>
              </div>
              <div style={s.impactItem}>
                <div style={s.impactBig}>{requests.length}</div>
                <div style={s.impactLabel}>Pending requests</div>
              </div>
              <div style={s.impactItem}>
                <div style={s.impactBig}>{cohorts.length}</div>
                <div style={s.impactLabel}>Cohorts led</div>
              </div>
            </div>
            <div style={s.card}>
              <div
        ...
        {page === 'cohorts' && (
          <>
            <div style={s.ph}>
              <div style={s.pt}>My cohorts</div>
              <div style={s.ps}>Small groups you are leading</div>
            </div>
            {cohorts.length === 0 && (
              <div style={{ fontSize: 13, color: C.muted, marginTop: 20 }}>
                No cohorts yet.
            ...
            {cohorts.map((c) => (
              <div key={c.id} style={s.card}>
                ...
                <div style={{ fontSize: 11, color: C.hint, marginBottom: 10 }}>
                  Next session: {c.next_session || 'TBD'}
                </div>
                <div style={{ fontSize: 12, color: C.muted }}>
                  {c.cohort_members?.length || 0} of {c.max_spots} spots filled
                </div>
              </div>
            ))}
          </>
